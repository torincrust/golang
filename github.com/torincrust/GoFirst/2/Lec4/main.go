...
func main() {
	...
	/* 	name := "кирилл"
	   	...
	   	fmt.Println("Length of string :", name, len(name)) // Функция len()(вместо ленс) возвращает количество элементов в наборе// если строка на русском то строка длиннее
	...
	fmt.Println(strings.Contains(totalString, subString)) // проверяет на наличие подстроки в строке
	// Rune -> alias int32
	var sampleRune rune
	var anotherRune rune = 'Q' // руны пишутяс в одинарных ковычках
	var thirdRune rune = 323
	...
	// сравнение строк
	fmt.Println(strings.Compare("abcd", "abcde")) // -1 если первая меньше, 0 если равны, 1 если первая больше
